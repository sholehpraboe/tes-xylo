        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="table w-full p-2">
                     <!-- component -->
                    <h3 class="text-gray-500 mb-1 text-xl font-bold uppercase my-4 pl-6">Edit Contact</h3>
                    @foreach($contacts as $value)
                    <form action="/contact/{{ $value->id }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="bg-white shadow rounded-lg p-6">
                            ...
                            <div class="grid lg:grid-cols-2 gap-6 my-4">
                                <div class="focus-within:text-blue-500 transition-all duration-500 relative ">
                                    ...
                                    <input id="name" name="name" autocomplete="false" tabindex="0" type="text" class="py-1 px-1 text-gray-900 block h-full w-full" value="{{ old('name') ? old('name') : $value->name }}">
                                </div>
                            </div>
                            <div class="grid lg:grid-cols-2 gap-6 my-6">
                                <div class="focus-within:text-blue-500 transition-all duration-500 relative ">
                                    ...
                                    <input id="phone" name="phone" autocomplete="false" tabindex="0" type="text" class="py-1 px-1 text-gray-900 block h-full w-full" value="{{ old('phone') ? old('phone') : $value->phone }}">
                                </div>
                            </div>
                            <div class="grid lg:grid-cols-2 gap-6 my-6">
                                <div class="focus-within:text-blue-500 transition-all duration-500 relative ">
                                    ...
                                    <input id="email" name="email" autocomplete="false" tabindex="0" type="text" class="py-1 px-1 text-gray-900 block h-full w-full" value="{{ old('email') ? old('email') : $value->email }}">
                                </div>
                            </div>
